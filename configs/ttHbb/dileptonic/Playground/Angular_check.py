...
def find_mother(progenitorId):
	...
	#Boolean array with particles from mother
...
particle_mass = {"b": 4.18, "electron": 0.0005110, "nu_e": 0.0, "muon": 0.105658, "nu_mu": 0.0, "top": 172.5}
mother_idx = GP.GenPart_genPartIdxMother
...
is_top = GP.GenPart_pdgId == 6
is_anti_top = GP.GenPart_pdgId == -6
is_Higgs = GP.GenPart_pdgId == 25
...
# top quark
top_mask = (from_hard_process & is_last_copy & is_top) != 0

GP["top"] = ak.zip(
    {
        "pt": GP.GenPart_pt[top_mask],
        "eta": GP.GenPart_eta[top_mask],
        "phi": GP.GenPart_phi[top_mask],
        "mass": GP.GenPart_mass[top_mask]
    },
    with_name="PtEtaPhiMCandidate"
)

# b quark from top
anti_top_mask = (from_hard_process & is_last_copy & is_anti_top) != 0

GP["anti_top"] = ak.zip(
    {
        "pt": GP.GenPart_pt[anti_top_mask],
        "eta": GP.GenPart_eta[anti_top_mask],
        "phi": GP.GenPart_phi[anti_top_mask],
        "mass": GP.GenPart_mass[anti_top_mask]
    },
    with_name="PtEtaPhiMCandidate"
)

# b quark from top
Higgs_mask = (from_hard_process & is_last_copy & is_Higgs) != 0

GP["Higgs"] = ak.zip(
    {
        "pt": GP.GenPart_pt[Higgs_mask],
        "eta": GP.GenPart_eta[Higgs_mask],
        "phi": GP.GenPart_phi[Higgs_mask],
        "mass": GP.GenPart_mass[Higgs_mask]
    },
    with_name="PtEtaPhiMCandidate"
)
...
particle3_mask = (
    (ak.num(GP["top"]) > 0)
    & (ak.num(GP["anti_top"]) > 0)
    & (ak.num(GP["Higgs"]) > 0)
#    & (ak.num(b_jets) > 3)
)


top_3p = GP["top"][particle3_mask]
anti_top_3p = GP["anti_top"][particle3_mask]
Higgs_3p = GP["Higgs"][particle3_mask]
...
top_3p_LV = vector.zip( {"pt": top_3p['pt'], "eta": top_3p['eta'], "phi": top_3p['phi'], "mass": top_3p['mass']})
anti_top_3p_LV = vector.zip( {"pt": anti_top_3p['pt'], "eta": anti_top_3p['eta'], "phi": anti_top_3p['phi'], "mass": anti_top_3p['mass']})
Higgs_3p_LV = vector.zip( {"pt": Higgs_3p['pt'], "eta": Higgs_3p['eta'], "phi": Higgs_3p['phi'], "mass": Higgs_3p['mass']})
...
TTbar_phi = abs( top_3p_LV.deltaphi(anti_top_3p_LV) )
THiggs_phi = abs( top_3p_LV.deltaphi(Higgs_3p_LV) )
TbarHiggs_phi = abs( anti_top_3p_LV.deltaphi(Higgs_3p_LV) )
...
ttH_ZMF = top_3p_LV + anti_top_3p_LV + Higgs_3p_LV

top_ttH_ZMF = top_3p_LV.boostCM_of_p4(ttH_ZMF)
anti_top_ttH_ZMF = anti_top_3p_LV.boostCM_of_p4(ttH_ZMF)
H_ttH_ZMF = Higgs_3p_LV.boostCM_of_p4(ttH_ZMF)
...
filter_phi = [x for x in min_Phi_ttH_ZMF if x is not None]
#print(filter_phi)
